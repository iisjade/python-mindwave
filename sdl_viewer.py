  ...
  for event in pygame.event.get():
    ...
    if event.type==KEYDOWN:
      if event.key== K_F5:
        ...
        file_suffix_csv = ".csv"
        file_suffix_pkt = ".pkt"
        file_name = dirpath + time.strftime('%Y%m%d%H%M%S') + file_suffix_csv
        packet_file_name = dirpath + time.strftime('%Y%m%d%H%M%S') + file_suffix_pkt
        p.start_raw_recording(file_name)
        p.start_packet_recording(packet_file_name)
      elif event.key == K_F8:
        p.stop_raw_recording()
        p.stop_packet_recording()
